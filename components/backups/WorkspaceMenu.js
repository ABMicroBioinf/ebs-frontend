...
export default function WorkspaceMenu({
  ...
  const getWorkList = () => {
    if (activeWork === "Analysis") {
      ...
      return (
        analyses &&
        analyses.map((ana) => <List.Item key={ana.id}>{ana.name}</List.Item>)
      );
    } else if (activeWork === "Sequence") {
      return (
        sequences &&
        sequences.map((seq) => (
          <List.Item key={seq.id}>{seq.seqfile_path}</List.Item>
        ))
      );
    } else {
      ...
      <Grid.Row columns={2} stretched>
        <Grid.Column width={5}>
          <Menu
            icon="labeled"
            vertical
            className="ebs-workspaces-toolbar-borderless"
          >
            <Menu.Item
              name="Analysis"
        ...
        <Grid.Column width={11}>
          <List>
            ...
            <List.Item>
              <List.List>{activeWork && getWorkList()}</List.List>
            </List.Item>
          </List>
        </Grid.Column>
      </Grid.Row>
